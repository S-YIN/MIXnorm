#MIXnorm: RNA-seq data normalization
#MIXnorm relies on a two-component mixture model
#Expressed genes are modeled by truncated normal distributions (in log scale)
#Non-expressed genes are modeled by zero-inflated Poisson distributions
#Function dzip returns the log likelihood for ZIP 
#Function MIXnorm is the main function for finding the MLE of the mixture model by nested EM algorithm
#Function func_MIXnorm returns the normalized data, proportion of expressed genes and the latent vairable D for each gene


#install.packages("truncnorm")
library(truncnorm)

#log likelihood for zero inflated Poisson data
#x is a matrix, x[j,i] follows ZIP(delta[i],pi_j[j])
#delta is a vector for poisson means, pi_j is a vector for prob. of extra zero
dzip <- function(x, delta,pi_j)
{
  #zip_llh = ZIP log-likelihood
  zip_llh <- matrix(NA,dim(x)[1],dim(x)[2])
  for (j in 1: dim(x)[1])
  {
    indictr <- ifelse(x[j,]>0,1,0)
    if (sum(indictr)!=0) #if data contain zeros, use ZIP pmf
    {
      zip_llh[j,] <- indictr*(log(1-pi_j[j])+dpois(as.numeric(x[j,]),delta,T))+(1-indictr)*(log(pi_j[j]+(1-pi_j[j])*(exp(-delta))))
    }
    if (sum(indictr)==0) #if no zero, use Poisson pmf
    {
...
MIXnorm <- function(dat, max_iter, tol)
{
  ...
  mu_i <- mu_i_final <- colMeans(log_dat)  # normal mean for each patient (include patient effect)
  sigma_i <- sigma_i_final <- apply(log_dat, 2, sd) # normal standard deviation
  zip_pi <- pi_j <- 1-I/samp_no #proportion of 0's from point mass of each gene
  phi <- nonzero_prop <- mean(pi_j < .5) #proportion of genes that are truely expressed
  delta <- poi_mean <- colMeans(dat[pi_j > .5,]) #poisson means for background read of genes not expressed
  delta[delta==0] <- 0.001 
  ...
  log_numtor <- apply(matrix(log(dtruncnorm(t(log_dat),a=0,b=Inf, mu_i, sigma_i)),dim(log_dat)[2],dim(log_dat)[1]), 2, sum)
  ...
  while (tolerance > tol)
  {
    ...
    while(K<=5)
    {
      ...
      if (K!=5)  #inner E (E2) step for Z (ZIP component)
      {
        for (i in 1:dim(dat)[2])
        {
          Z[,i] <- pi_j^(1-D)/(pi_j^(1-D)+(1-pi_j)^(1-D)*exp(-delta[i]*(1-D)))
    ...
    #outer E step
    log_numtor <- apply(matrix(log(dtruncnorm(t(log_dat),a=0,b=Inf, mu_i_final, sigma_i_final)),dim(log_dat)[2],dim(log_dat)[1]), 2, sum)
    log_dentor1 <- dzip(x=dat,delta=delta,pi_j=pi_j)
    ...
    tolerance <- max(abs(mu_i_final-mu_i),abs(sigma_i_final-sigma_i),abs(nonzero_prop-phi),abs(zip_pi-pi_j),abs(poi_mean-delta))
    print(tolerance) #print the maximum parameter change between current and previous iteration
    ...
    if (iter > max_iter)
    {
      warning(paste('Algorithm exceeded maximum iteration (', max_iter, ') and stoped.'))
      break
    }
  }
  
  if (iter <= max_iter) cat(paste0('\n','Algorithm converges at ', iter,'th iteration.\n' ))
